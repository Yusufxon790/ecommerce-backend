...
class StatsController extends Controller
{
    ...
    public function ordersCount(Request $request)
    {
        ...
        return $this->response(Order::query()->whereBetween('created_at',[$from,Carbon::parse($to)->endOfDay()])
    ...
    public function deliveryMethodsRatio(Request $request)
    {
        ...
        foreach (DeliveryMethod::all() as $deliveryMethod) {
            $deliveryMethodOrders = $deliveryMethod->orders()->whereBetween('created_at',[$from,Carbon::parse($to)->endOfDay()])
            ->whereRelation('status','code','closed')
            ->count();
    ...
    public function ordersCountByDays(Request $request){
        ...
        $count=Order::query()->whereDate('created_at',$day)
        ->whereRelation('status','code','closed')
        ->count();
